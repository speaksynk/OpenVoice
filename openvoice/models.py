...
class PosteriorEncoder(nn.Module):
    def __init__(
        ...
        self.pre =  nn.Conv1d(in_channels, hidden_channels, 1).to('xla')
        self.enc = modules.WN(
            hidden_channels,
            kernel_size,
            dilation_rate,
...
class SynthesizerTrn(nn.Module):
    ...
    #  y_lengths, sid_src, sid_tgt, g_zeros_src, g_zeros_tgt,
    def voice_conversion(self, y, y_lengths, sid_src, sid_tgt, g_zeros_src, g_zeros_tgt):
        tau=0.3
        g_src = sid_src
        g_tgt = sid_tgt
        z, m_q, logs_q, y_mask = self.enc_q(y, y_lengths, g=g_zeros_src, tau=tau)
        z_p = self.flow(z, y_mask, g=g_src)

        z_hat = self.flow(z_p, y_mask, g=g_tgt, reverse=True)
        o_hat = self.dec(z_hat * y_mask, g=g_zeros_tgt)
